...
export async function generateMetadata({ params }: TrideviPageProps): Promise<Metadata> {
  ...
  if (!goddess) {
    return {
      ...
      description: 'The requested goddess page could not be found.',
    };
  }

  const siteName = 'Shakti Darshan';
  const pageTitle = `Maa ${goddess.name} - ${goddess.aspect} | ${siteName}`;
  const pageDescription = goddess.introduction.substring(0, 160) + (goddess.introduction.length > 160 ? '...' : '');
  const pageUrl = `/tridevi/${params.slug}`;
  const imageUrl = `https://picsum.photos/seed/${goddess.imageHint}/1200/630`;

  return {
    title: pageTitle,
    description: pageDescription,
    keywords: [goddess.name, goddess.aspect, 'Tridevi', 'Divine Feminine', 'Hindu Goddess', ...goddess.attributes.slice(0,3)],
    openGraph: {
      title: pageTitle,
      description: pageDescription,
      url: pageUrl,
...
export default function TrideviPage({ params }: TrideviPageProps) {
  ...
  if (!goddess) {
    return <div className="container mx-auto px-4 py-8 text-center">Goddess not found.</div>;
  }
  ...
  return (
    <div className="container mx-auto px-4 py-10">
      <header className="mb-12 text-center">
        ...
        <h1 className="text-4xl md:text-6xl font-bold text-primary mb-2 drop-shadow-md">{goddess.name}</h1>
        ...
        <div className="lg:col-span-1">
          <Card className="sticky top-10 shadow-xl rounded-xl overflow-hidden border-2 border-primary/30 transform transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
            <CardHeader className="p-0">
              <Image
                src={`https://picsum.photos/seed/${goddess.imageHint}/600/700`}
                alt={goddess.name}
                width={600}
                height={700}
            ...
            <CardContent className="p-6 bg-gradient-to-b from-card to-muted/20">
              ...
              {(goddess.detailedMantrasAndStotras?.mantras ? goddess.detailedMantrasAndStotras.mantras.slice(0,2) : goddess.mantras).map((mantra, index) => (
                <Badge 
                  key={index} 
                  variant="outline" 
                  className="block w-full text-left text-sm p-3 my-2 font-mono shadow-sm break-words whitespace-normal hover:bg-accent/10"
                >
                  {typeof mantra === 'string' ? mantra : mantra.name}
                  {typeof mantra !== 'string' && mantra.sanskrit && <span className="block text-xs opacity-80 mt-1">{mantra.sanskrit}</span>}
                </Badge>
              ))}
            </CardContent>
          </Card>
      ...
      {/* Durga-specific sections using detailed data */}
      {isDurga && goddess.avatarsForms && (
        <section className="mb-12">
            <h2 className="text-3xl font-semibold text-center mb-8 text-secondary flex items-center justify-center gap-2"><Sparkles className="w-8 h-8" /> Avatars & Forms of Maa Durga</h2>
      ...
      {goddess.slug === 'durga' && goddess.navadurga && (
        <section className="mb-12">
          <h2 className="text-3xl font-semibold text-center mb-8 text-secondary flex items-center justify-center gap-2"><ShieldQuestion className="w-8 h-8" /> The Navadurga</h2>
                ...
                <Card className="shadow-xl rounded-xl overflow-hidden border border-primary/20">
                  ...
                  <CardContent className="md:flex md:gap-6 items-start p-6">
                     <div className="md:w-1/3 mb-4 md:mb-0 flex-shrink-0">
                       <Image
                          src={`https://picsum.photos/seed/${form.imageHint.toLowerCase().replace(/\s+/g, '-')}/400/500`}
                          alt={form.name}
                          width={400}
                          height={500}
      ...
      {goddess.slug === 'lakshmi' && goddess.ashtaLakshmi && (
         <section className="mb-12">
          <h2 className="text-3xl font-semibold text-center mb-8 text-secondary flex items-center justify-center gap-2"><HandCoins className="w-8 h-8" /> The Ashta Lakshmi</h2>
      ...
      {/* Saraswati Aspects section */}
      {goddess.slug === 'saraswati' && goddess.saraswatiAspects && (
        <section className="mb-12">
          <h2 className="text-3xl font-semibold text-center mb-8 text-secondary flex items-center justify-center gap-2"><Edit3 className="w-8 h-8" /> Aspects of Maa Saraswati</h2>
              ...
              <CardContent className="md:flex md:gap-4 items-center">
                {aspect.imageHint && (
                  <div className="md:w-1/4 mb-4 md:mb-0 flex-shrink-0">
                    <Image
                        src={`https://picsum.photos/seed/${aspect.imageHint.replace(/\s+/g, '-')}/300/200`}
                        alt={aspect.title}
                        width={300}
                        height={200}
       ...
       <Separator className="my-12" />
        ...
        <CardTitle className="text-2xl text-accent mb-3 flex items-center justify-center gap-2"><HandHeart className="w-8 h-8" /> Path to Divine Understanding</CardTitle>
