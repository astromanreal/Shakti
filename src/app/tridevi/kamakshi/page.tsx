...
const goddess = allMajorFormsDetailedData.find((g) => g.id === 'kamakshi');

export async function generateMetadata(): Promise<Metadata> {
  if (!goddess) {
    return {
      title: 'Kamakshi Not Found | Shakti Darshan',
      description: 'The requested Kamakshi page could not be found.',
  ...
  const pageUrl = `${siteUrl}/tridevi/kamakshi`; // Static URL for this page
...
const IconMapPageSpecific: Record<string, React.ElementType> = {
  Eye, ShieldCheck, Gem, Gift, Sparkles, Sword, Flame, Heart, Default: Eye // Defaulting to Eye for Kamakshi if iconName is Eye
};

export default function KamakshiStaticPage() {
  if (!goddess) {
    return (
        <div className="container mx-auto px-4 py-12 flex flex-col items-center justify-center min-h-[calc(100vh-10rem)]">
          <Card className="w-full max-w-lg text-center shadow-xl p-8 bg-destructive/5 border-destructive">
            <CardTitle className="text-3xl font-bold text-destructive mb-4">Kamakshi Not Found</CardTitle>
            <CardDescription className="text-lg text-foreground/80 mb-6">
              Details for Goddess Kamakshi could not be loaded.
            </CardDescription>
  ...
  const HeroIcon = IconMapPageSpecific[goddess.iconName || 'Default'] || Eye;
  ...
  const articleSchema = {
    ...
    "mainEntityOfPage": { "@type": "WebPage", "@id": `${siteUrl}/tridevi/kamakshi` },
