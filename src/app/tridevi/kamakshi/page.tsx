...
// This file is now a dynamic page-like component for Kamakshi.
// It fetches its own data from the consolidated source.

const goddess: OtherMajorFormDetail | undefined = allMajorFormsDetailedData.find((g) => g.id === 'kamakshi');
const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.shaktidarshan.com';

const pageTitle = goddess ? `${goddess.name} - ${goddess.title} | Shakti Darshan` : "Maa Kamakshi | Shakti Darshan";
const pageDescription = goddess ? `${goddess.description?.substring(0, 155)}... Explore her iconography, mantras, temples, and philosophical significance.` : "Explore details about Maa Kamakshi, a prominent form of the Divine Mother.";
const imageUrl = goddess?.detailImageUrl || goddess?.imageUrl || `${siteUrl}/og-images/tridevi/forms/kamakshi.jpg`;

export const metadata: Metadata = {
    title: pageTitle,
    description: pageDescription,
    keywords: ["Kamakshi", "Sri Vidya", "Kanchipuram", "Lalita", "Tripurasundari", "Divine Feminine"],
    alternates: {
        canonical: '/tridevi/forms/kamakshi',
    },
    openGraph: {
        title: pageTitle,
        description: pageDescription,
        url: `${siteUrl}/tridevi/forms/kamakshi`,
...
export default function KamakshiPage() {
    if (!goddess) {
        return (
            <div className="container mx-auto px-4 py-12 flex flex-col items-center justify-center min-h-[calc(100vh-10rem)]">
                <Card className="w-full max-w-lg text-center shadow-xl p-8 bg-destructive/5 border-destructive">
                    ...
                    <CardDescription className="text-lg text-foreground/80 mb-6">
                        The divine form of Maa Kamakshi is not currently detailed.
                    </CardDescription>
    ...
    const HeroIcon = Eye; // Kamakshi's icon
